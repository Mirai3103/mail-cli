...
export class GmailProvider implements EmailProviderPort {
	...
	async list(
		...
		try {
			...
			throw new CLIError("GMAIL_API_ERROR", "Failed to list messages", err);
		}
	}
	...
	async send(msg: SendEmailOptions): Promise<string> {
		try {
			...
			if (err instanceof CLIError) throw err;
			throw new CLIError("GMAIL_API_ERROR", "Failed to send message", err);
		}
	}
	...
	async status(): Promise<{ unread: number; total: number }> {
		try {
			...
			if (!inboxLabel) {
				throw new CLIError("GMAIL_API_ERROR", "INBOX label not found");
			}
