...
describe("TokenStorageImpl (integration)", () => {
	...
	afterEach(async () => {
		...
		try {
			await fs.rm(path.join(TEST_TOKENS_DIR, testSubDir), {
				recursive: true,
				force: true,
			});
		} catch {
	...
	test("getTokens returns null for non-existent account", async () => {
		const result = await storage.getTokens(
			`${testEmailPrefix}nonexistent-${Date.now()}@example.com`,
		);
	...
	test("listAccounts returns all account IDs from tokens directory", async () => {
		await storage.saveTokens(`${testEmailPrefix}user1@example.com`, {
			access_token: "a",
		});
		await storage.saveTokens(`${testEmailPrefix}user2@example.com`, {
			access_token: "b",
		});
