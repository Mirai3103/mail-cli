...
function probeControllingTtyToken() {
	...
	try {
		...
		if (ttyPath && ttyPath !== "not a tty") {
			const token = sanitizeWorkstreamSessionToken(
				ttyPath.replace(/^\/dev\//, ""),
			);
...
function getActiveWorkstream(cwd) {
	...
	if (sessionScoped) {
		return readActiveWorkstreamPointer(
			sessionScoped.filePath,
			cwd,
			sessionScoped.dirPath,
		);
	}
...
function setActiveWorkstream(cwd, name) {
	...
	if (!name) {
		clearActiveWorkstreamPointer(
			filePath,
			sessionScoped ? sessionScoped.dirPath : null,
		);
		return;
	}
...
function extractPhaseToken(dirName) {
	...
	const codePrefixed = dirName.match(
		/^([A-Z]{1,6}-\d+[A-Z]?(?:\.\d+)*)(?:-|$)/i,
	);
