...
function readModifyWriteStateMd(statePath, transformFn, cwd) {
	const lockPath = acquireStateLock(statePath);
	try {
		const content = fs.existsSync(statePath)
...
function updatePerformanceMetricsSection(
	...
	const prevTotal =
		totalMatch && totalMatch[1] !== "[N]" ? parseInt(totalMatch[1], 10) : 0;
	...
	if (byPhaseMatch) {
		...
		const phaseRowPattern = new RegExp(
			`^\\|\\s*${escapeRegex(String(phaseNum))}\\s*\\|.*$`,
			"m",
		);
	...
	if (currentPhase && fs.existsSync(phasesDir)) {
		...
		try {
			...
			const phaseDir = entries.find(
				(e) =>
					e.isDirectory() &&
					e.name.startsWith(normalized.replace(/^0+/, "").padStart(2, "0")),
			);
			if (phaseDir) {
				...
				const diskSummaries = files.filter((f) =>
					f.match(/-SUMMARY\.md$/i),
				).length;
				...
				for (const vf of verificationFiles) {
					try {
						const vContent = fs.readFileSync(
							path.join(phaseDirPath, vf),
							"utf-8",
						);
						if (
							/status:\s*passed/i.test(vContent) &&
							/executing/i.test(status)
						) {
							warnings.push(
								`Status drift: STATE.md says "${status}" but ${vf} shows verification passed — phase may be complete`,
							);
							drift.verification_status = {
								state_status: status,
								verification: "passed",
							};
						}
...
function cmdStateSync(cwd, options, raw) {
	...
	if (highestIncompletePhase) {
		const currentPlansField = stateExtractField(
			modified,
			"Total Plans in Phase",
		);
		if (
			currentPlansField &&
			parseInt(currentPlansField, 10) !== highestIncompletePhaseplanCount
		) {
			changes.push(
	...
	const percent =
		totalDiskPlans > 0
			? Math.min(100, Math.round((totalDiskSummaries / totalDiskPlans) * 100))
			: 0;
