...
function detectProjectType(cwd) {
	...
	let has_tests =
		exists("test") || exists("tests") || exists("__tests__") || exists("spec");
	if (!has_tests) {
		try {
			const pkg = JSON.parse(
				fs.readFileSync(path.join(cwd, "package.json"), "utf-8"),
			...
			has_tests = devDeps.some((d) =>
				["vitest", "jest", "mocha", "jasmine", "ava"].includes(d),
			);
		} catch {
			/* ignore */
		}
...
function detectDocTooling(cwd) {
	...
	return {
		docusaurus:
			exists("docusaurus.config.js") || exists("docusaurus.config.ts"),
		vitepress:
...
function detectMonorepoWorkspaces(cwd) {
	...
	try {
		const lerna = JSON.parse(
			fs.readFileSync(path.join(cwd, "lerna.json"), "utf-8"),
		);
