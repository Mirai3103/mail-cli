...
const ORM_INFO = {
	...
	prisma: {
		...
		envHint:
			"npx prisma db push --accept-data-loss (if destructive changes are intended)",
		interactiveWarning:
...
function checkSchemaDrift(changedFiles, executionLog, options = {}) {
	...
	for (const orm of detection.orms) {
		...
		const hasPushEvidence = info.evidencePatterns.some((p) =>
			p.test(executionLog),
		);
		if (hasPushEvidence) {
			pushedOrms.add(orm);
	...
	if (skipCheck) {
		return {
			...
			unpushedOrms,
			message:
				"Schema drift detected but check was skipped (GSD_SKIP_SCHEMA_CHECK=true).",
		};
	}
